...
def send_view(link, proxy):
    ...
    try:
        ...
        main_res = session.get(link, timeout=10)
        main_res.raise_for_status()

        _token = main_res.text.split('data-view="')[1].split('"')[0]
        views_req = session.get("https://t.me/v/?views=" + _token, timeout=10)
...
if __name__ == "__main__":
    try:
        ...
        port = 33335 # For SOCKS proxies (if used)
        url_fin = f'https://{link}?embed=1'

        proxies = read_proxies(proxy_type) if proxy_type != "none" else []
...
Types: http, socks4, socks5, mix, none""")
    except ValueError:
